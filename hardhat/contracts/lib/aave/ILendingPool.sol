...
interface ILendingPool {
   ...
   * - Send the value type(uint).max in order to repay the whole debt for `asset` on the specific `debtMode`
  ...
  function setFNFTConfiguration(address reserve, uint configuration) external;
   ...
   **/
  function getFNFTConfiguration(address asset)
    external
    view
    returns (DataTypes.ReserveFNFTConfigurationMap memory);
   ...
   **/
  function getUserFNFTConfiguration(address user)
    external
    view
    returns (DataTypes.UserFNFTConfigurationMap memory);
